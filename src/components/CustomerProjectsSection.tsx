...
export const CustomerProjectsSection: React.FC<CustomerProjectsSectionProps> = ({
  ...
  const loadData = async () => {
    ...
    // Find projects assigned to this customer or matching customer_email
    ...
    // If user has no specific project assigned yet, show projects they might be participating in
    // or provide the demonstrative projects clearly marked
    const displayProjects = userProjects.length > 0 ? userProjects : allProjects.slice(0, 2);

    setProjects(displayProjects);
    setTimeEntries(allTime);
    if (displayProjects.length > 0) {
      setExpandedProjectId(displayProjects[0].id);
    }
    setLoading(false);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <h3 className="text-xl font-bold text-white font-['Outfit'] flex items-center gap-2">
            ...
            <span>Project Management &amp; Progress Tracking</span>
          </h3>
          <p className="text-xs text-slate-400 mt-1">
            Real-time status updates, milestones, and verified manual work time logged by
            SrijanTech.
          </p>
        </div>
      ...
      {projects.length === 0 ? (
        <div className="p-8 rounded-2xl bg-slate-900/60 border border-slate-800 text-center space-y-2">
          <p className="text-sm text-slate-300 font-semibold">No active projects assigned yet</p>
          <p className="text-xs text-slate-400 max-w-md mx-auto">
            Once your project brief is approved and advance confirmation is complete, your dedicated
            project roadmap, milestones, and work time summary will be displayed here.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          {projects.map((proj) => {
            ...
            const statusConfig = STATUS_LABELS[proj.status] || {
              label: proj.status.replace(/_/g, ' '),
            ...
            return (
              <div
                key={proj.id}
                className="rounded-3xl bg-slate-900/80 border border-slate-800 overflow-hidden shadow-xl"
              >
                {/* Project Header Bar */}
                <div
                  ...
                  <div className="space-y-1.5">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 rounded-md bg-slate-800 text-[10px] text-slate-400">
                        {proj.project_type || proj.category || 'Web Application'}
                      </span>
                    </div>
                ...
                {isExpanded && (
                  <div className="p-6 border-t border-slate-800 space-y-6 bg-slate-950/40">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 rounded-2xl bg-slate-900/60 border border-slate-800 text-xs">
                      <div>
                        <span className="text-slate-400 block text-[10px] uppercase font-semibold">
                          Start Date
                        </span>
                        <span className="text-white font-medium">
                          {proj.start_date || '2025-01-05'}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[10px] uppercase font-semibold">
                        ...
                        <span className="text-white font-medium">
                          {proj.expected_completion || proj.target_delivery_date || '2025-02-15'}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[10px] uppercase font-semibold">
                    ...
                    {/* Progress Bar for Mobile / High Resolution */}
                    <div className="space-y-1.5">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                          { title: '1. Architecture & DB Schema', status: 'completed' },
                          { title: '2. Core UX & Wireframes', status: 'completed' },
                          {
                            title: '3. API & Payment Integration',
                            status: progress >= 60 ? 'completed' : 'in_progress',
                          },
                          {
                            title: '4. QA & Security Hardening',
                    ...
                    <div className="space-y-3 pt-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-slate-400">
                          Total: <strong className="text-white">{totalHours}h {remMinutes}m</strong>
                        </span>
                      </div>
